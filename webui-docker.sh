...
# git clone the requirement/dependency
...
pip3 install --root-user-action ignore torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu121 && \
pip3 install --root-user-action ignore -r requirements_versions.txt && \
pip3 install --root-user-action ignore joblib && \
